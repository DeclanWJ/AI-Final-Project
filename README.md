...
**Files:**
 ...
 The script for the learning agent makes use of several java class files in order to interact with the game client and learn how to gather resources through the OSBot software, and one text file to store any learned information about states accumulated during runtime. In the below descriptions of the files included in this project, the methods we found particularly important to the overall functionality of our code are mentioned.
 
 **Artificial_Intelligence_Project.java** - This file is the main file used by the script, containing all functionality and code that interacts with the OSBot client to gather information from the game space. This file also contains toggeable debugging information displays both actively updating on screen and in the OSBot client's logger. Determining legal actions, executing selected actions, as well as keeping track of the length of gathering episodes all occur in this class. Additionaly, due to the structure of the scripts used by OSBot, we had to place several methods in this class in order for them to function properly as they relied on updated information gathered directly from the game client interface. This included methods for calculating the state an agent would be in given a selected action (getNextState(**State currentState**, String actionLabel)), and determining rewards for taking those actions to be used in Q-value calculations (getReward(**State currentState**, String actionLabel)). The method createMapFromRecord(), which is used to create a hash map matching the structure of the map used to store information about states from data stored in the trainingData.txt file (this being used to essentially handle importing all stored data from previous learning for use in current script execution), is also contained in this class.
 
 **QLearning_Agent.java** - This class is responsible for storing learned Q-values in a hash map making use of positions in Old School Runescape and action labels to represent states. When starting without data, this class will handle creation of the hashmap to hold "blank" values for every position in the area an agent is restricted to. When data from previous learning is detected it is used to overwrite appropriate state information for positions recorded in the data. Calculation of optimal actions in a given state, actions chosen when the exploration rate comes into play, as well as updating of Q-values is handled in this class. The method computeActionFromQValues(State state) is present in several other methods in the QLearning_Agent.java class; it looks at all Q-values in a given state and returns the legal action with the highest Q-value, but if every action has the same value a random legal action is selected. The method computeValueFromQValues(State state), for example uses the calculated "bestAction" in order to find the "best" Q-value of the state it's passed. checkIfKnown(State state) is the method used when initializing the hash map that is going to store all Q-value information for the states in the area the agent is restricted to. Finally the update(State state, String actionLabel, State nextState, double reward) method that handles the actually mathematical backbone of the agent's learning is also housed in this class.
